...
CollabArt is a collaborative music platform where musicians and artists create projects, upload audio files, manage collaborations, submit/review lyrics, list projects for sale, and send collaboration invites. It runs on **Next.js 14 (App Router)** with **Convex** as the serverless backend and **Clerk** for authentication.
...
okay now i want you to help set up claude code so that different code agent can be able to pick up from where i last stopped working with maybe a current agent due to rate limit etc., i want this setup in a way the memory can sync as much as possible so that different coding agent can continue the same agent, that is since claude already have the plan written, i can execute with other agents if its recorded in memory, so this kind of infrastructure is setup accross all projects. firstly lets ruminate deeply on how we can set that. basically i want to use cross platfroms in devlopment, that is using codex or opencode when claude code reaches rate limit but i want this to be synchromized with other coding ai agents so that they can all register current workflow and where it stop so other agents know where and how to proceed development no matter what. following already set plans